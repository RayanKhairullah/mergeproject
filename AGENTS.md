...
- Strict Requirement: Always use Laravel 12 syntax.
- Documentation: Use the `search-docs` tool for v12 specific queries.
- Structure: Follow the streamlined directory structure (no Http/Kernel.php, no Console/Kernel.php).
...
- Use Spatie's `HasRoles` trait in the User model.
- When checking permissions in Volt components, use the `can` middleware or `$user->can()` method.
...
- Strict Requirement: Always use the Functional API for Volt components.
...
### Project Design System (v4.2 Brand)
- **Color Space:** Use P3 Wide Gamut colors via `oklch()` for brand vibrancy.
- **Brand Accent:** Use `mint` as the primary accent color.
    - Base: `--color-mint-500: oklch(0.7 0.28 145);`
    - Deep: `--color-mint-950: oklch(0.3 0.4 145);`
- **Typography:** - Heading style: Always use `tracking-tighter` and `text-balance` for all titles (`text-4xl` and above).
    - Font Primary: "Inter", Font Mono: "IBM Plex Mono".
- **Visual Effects:** - Stack filters like `blur-sm` and `saturate-200` for modern glassmorphism.
    - Use `duration-750 ease-in-out` for all major transitions.
- **Responsive Layout:**
    - Prefer **Container Queries** (`@container`) over Viewport Queries (`sm:`, `md:`) for internal components.
    - Example: `<div class="@sm:grid-cols-2">` instead of `<div class="sm:grid-cols-2">`.
